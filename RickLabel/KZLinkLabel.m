...
- (void)setupTextSystem {
    ...
    self.linkBackgroundColor = [UIColor clearColor]; // [UIColor colorWithWhite:0.95 alpha:1.0];
    self.linkColor = RGBCOLOR(0, 240, 255);
    self.linkHighlightColor = [UIColor orangeColor];
    self.linkUnderlineStyle = NSUnderlineStyleSingle;
...
- (void)setLinkUnderlineStyle:(NSUnderlineStyle)linkUnderlineStyle {
    _linkUnderlineStyle = linkUnderlineStyle;
    [self updateTextStoreWithText];
}
...
- (void)updateTextStoreWithAttributedString:(NSAttributedString *)attributedString {
    ...
    self.KZAttributedString = attributedString;
...
//                                 NSUnderlineStyleAttributeName : @(self.linkUnderlineStyle)
//                                 };
    ...
    for (NSDictionary *dictionary in linkRanges) {
        ...
        if ((KZLinkType)[dictionary[@"linkType"] intValue] == KZLinkTypeURL ||
            (KZLinkType)[dictionary[@"linkType"] intValue] == KZLinkTypeHTMLHref) {
            [attributedString addAttribute:NSUnderlineStyleAttributeName
                                     value:@(self.linkUnderlineStyle)
                                     range:range];
        }
...
- (NSArray *)getRangesForLinks:(NSAttributedString **)text {
    ...
    //HTML超链接(将改变内容text)
    if (self.linkDetectionTypes & KZLinkDetectionTypeHTMLHref) {
        [rangesForLinks addObjectsFromArray:[self getRangesForHTMLHrefs:text]];
    }
    
    //用户昵称
    if (self.linkDetectionTypes & KZLinkDetectionTypeUserHandle) {
        [rangesForLinks addObjectsFromArray:[self getRangesForUserHandles:plainText]];
    }
    //内容标签
    if (self.linkDetectionTypes & KZLinkDetectionTypeHashTag) {
        [rangesForLinks addObjectsFromArray:[self getRangesForHashTags:plainText]];
    }
    //链接地址
    ...
    if (self.linkDetectionTypes & KZLinkDetectionTypePhoneNumber) {
        [rangesForLinks addObjectsFromArray:[self getRangesForPhoneNumbers:plainText]];
    }
    //......
 ...
 * 所有HTML链接地址
 */
- (NSArray *)getRangesForHTMLHrefs:(NSAttributedString **)text {
    NSInteger lengthChange = 0;
    NSMutableArray *rangesForURLs = [[NSMutableArray alloc] init];;
    NSError *error = nil;
    NSString *plainText = (*text).string;
    NSString *pattern = @"<a\\s.*?href=\"([^\"]+)\"[^>]*>(.*?)</a>";
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:pattern
                                                                      options:0
                                                                        error:&error];
    
    NSArray *matches = [regex matchesInString:plainText
                                      options:0
                                        range:NSMakeRange(0, (*text).length)];
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        NSRange realRange = NSMakeRange(matchRange.location - lengthChange, contentRange.length);
//        contentRange.location = contentRange.location - self.lengthChange;
//        hrefRange.location = hrefRange.location - self.lengthChange;
        
        NSString *content = [plainText substringWithRange:contentRange];
        NSString *realURL = [plainText substringWithRange:hrefRange];
        
        matchRange.location = matchRange.location - lengthChange;
        ...
        *text = attributedString;
        ...
        lengthChange = matchRange.length - contentRange.length;
        if ([match resultType] == NSTextCheckingTypeLink ||
            [match resultType] == NSTextCheckingTypeRegularExpression) {
            [rangesForURLs addObject:@{
                                       @"linkType" : @(KZLinkTypeHTMLHref),
                                       @"range"    : [NSValue valueWithRange:realRange],
                                       @"link"     : realURL
                                       }];
        }
...
//    NSDataDetector *detector = [[NSDataDetector alloc] initWithTypes:NSTextCheckingTypeLink error:&error];
    NSString *plainText = text.string;
    NSString *pattern = @"[http|https]+[://]+[0-9A-Za-z:/[-]_[&]#[?][=][.]]*";
    NSRegularExpression *regex = [[NSRegularExpression alloc] initWithPattern:pattern
                                                                      options:0
                                                                        error:&error];
    NSArray *matches = [regex matchesInString:plainText
